...
public class PatientComposite extends Composite implements ClickHandler, ChangeHandler
{
	...
	public PatientComposite ()
	{
		...
		clientInfoTextArea.setWidth ("100%");
		...
		attributeComboBox.addItem ("FIRST NAME", "person;first_name");
		attributeComboBox.addItem ("LAST NAME", "person;last_name");
		attributeComboBox.addItem ("GENDER", "person;gender");
		attributeComboBox.addItem ("DATE OF BIRTH", "person;dob");
		attributeComboBox.addItem ("NATIONAL ID", "person;national_id");
		attributeComboBox.addItem ("PRIMARY CONTACT", "person;mobile");
		attributeComboBox.addItem ("SECONDARY CONTACT", "person;alternate_mobile");
		attributeComboBox.addItem ("TERTIARY CONTACT", "person;tertiary_mobile");
	...
	public void saveData ()
	{
		if (validate ())
		{
			...
			String attribute = IRZClient.get (attributeComboBox);
			String value = IRZClient.get (valueSimplePanel.getWidget ()).toUpperCase ();
			encounterResults.add (new EncounterResults (new EncounterResultsId (eId, clientId, pid2, formName, "ATTRIBUTE"), attribute));
