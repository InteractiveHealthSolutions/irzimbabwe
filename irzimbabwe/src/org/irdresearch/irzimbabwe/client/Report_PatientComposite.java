...
public class Report_PatientComposite extends Composite implements IReport, ClickHandler, ChangeHandler, ValueChangeHandler<Boolean>
{
	...
	private Label						lblTbReachLog					= new Label (IRZ.getProjectTitle () + " Patient Custom Search");
	...
	public void viewData (boolean export)
	{
		try
		{
			...
			if (monitorCheckBox.getValue ())
			{
				...
				filters.append ("and P.MonitorID = '" + IRZClient.get (monitorComboBox) + "' ");
			}
			if (gpCheckBox.getValue ())
			{
				columns.append ("P.GPID, ");
				filters.append ("and P.GPID = '" + IRZClient.get (gpComboBox) + "' ");
			}
			if (chwCheckBox.getValue ())
			{
				columns.append ("P.CHWID, ");
				filters.append ("and P.CHWID = '" + IRZClient.get (chwComboBox) + "' ");
			}
			...
			if (ageCheckBox.getValue ())
			{
				...
				filters.append ("and year(P.DateRegistered) - year(Pr.DOB) between " + IRZClient.get (ageFromTextBox) + " and " + IRZClient.get (ageToTextBox) + " ");
			}
			...
			if (categoryCheckBox.getValue ())
			{
				...
				filters.append ("and P.DiseaseCategory = '" + IRZClient.get (categoryComboBox) + "' ");
			}
			if (treatmentPhaseCheckBox.getValue ())
			{
				columns.append ("P.TreatmentPhase, ");
				filters.append ("and P.TreatmentPhase = '" + IRZClient.get (treatmentPhaseComboBox) + "' ");
			}
			if (regimenCheckBox.getValue ())
			{
				columns.append ("P.Regimen, ");
				filters.append ("and P.Regimen = '" + IRZClient.get (regimenComboBox) + "' ");
			}
			...
			{
				String option = IRZClient.get (smearMicroscopyOptionsComboBox);
			...
			if (followupsCheckBox.getValue ())
			{
				...
				if (IRZClient.get (followupMonthComboBox).equals ("MONTH 2 FOLLOWUP"))
					monthCriteria = "between 54 and 143";
				else if (IRZClient.get (followupMonthComboBox).equals ("MONTH 3 FOLLOWUP"))
					monthCriteria = "between 84 and 143";
				else if (IRZClient.get (followupMonthComboBox).equals ("MONTH 5 FOLLOWUP"))
					monthCriteria = "between 144 and 203";
				else if (IRZClient.get (followupMonthComboBox).equals ("MONTH 7 FOLLOWUP"))
					monthCriteria = ">= 204";
				if (IRZClient.get (followupSelectionComboBox).equals ("HAS"))
