...
public class Report_DashboardComposite extends Composite implements IForm, ClickHandler, ChangeHandler, SelectionHandler<Integer>
{
	...
	private Grid						patientsGrid				= new Grid (3, 2);
	private Grid						testsGrid					= new Grid (6, 2);

	private Button						refreshButton				= new Button ("Refresh");
	private Button						closeButton					= new Button ("Close");

	private Label						topLabel					= new Label (IRZ.getProjectTitle () + " Dashboard");
	...
	private Label						lblScreened					= new Label ("Clients:");
	...
	public Report_DashboardComposite ()
	{
		...
		patientsTextArea.setVisibleLines (5);
		patientsTextArea.setCharacterWidth (40);
		patientsTextArea.setReadOnly (true);
		patientsGrid.setWidget (2, 1, patientsTextArea);
	...
	private void displaySummary ()
	{
		...
		try
		{
			...
			{
				Widget w = iter.next ();
				String encounter = specifyFormTypeCheckBox.getValue () ? IRZClient.get (encounterTypeComboBox) : "";
				if (w instanceof TextBox)
				{
					TextBox textBox = (TextBox) w;
			...
			{
				String option = IRZClient.get (patientOptionsComboBox);
				String selection = IRZClient.get (patientOptionsListComboBox);
			...
			// Fill Screening
			sqlQueries.clear ();
			sqlQueries.add ("select count(*) from patient inner join person on person.pid = patient.patient_id where 1 = 1 " + filter);
			sqlQueries.add ("select count(*) from patient inner join person on person.pid = patient.patient_id where disease_suspected = 'TB' " + filter);
			sqlQueries.add ("select count(*) from patient inner join person on person.pid = patient.patient_id where disease_suspected = 'HIV' " + filter);
			sqlQueries.add ("select count(*) from patient inner join person on person.pid = patient.patient_id where disease_suspected = 'CM' " + filter);
			sqlQueries.add ("select count(*) from patient inner join person on person.pid = patient.patient_id where disease_confirmed = 1 " + filter);
			service.getQueriesResults (sqlQueries.toArray (new String[] {}), new AsyncCallback<String[]> ()
			{
				public void onSuccess (String[] result)
				{
					...
					builder.append ("TB Cases: " + result[1]);
					builder.append ("HIV Cases: " + result[2]);
					builder.append ("CM Cases: " + result[3]);
					builder.append ("TB Confirmed Cases: " + result[3]);
					patientsTextArea.setText (builder.toString ());
				}
	...
	public void onChange (ChangeEvent event)
	{
		...
		if (sender == patientOptionsComboBox)
		{
			...
			if (IRZClient.get (patientOptionsComboBox).equals ("AGE"))
			{
			...
			else if (IRZClient.get (patientOptionsComboBox).equals ("GENDER"))
			{
