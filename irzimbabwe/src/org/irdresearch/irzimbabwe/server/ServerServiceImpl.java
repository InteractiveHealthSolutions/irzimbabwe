...
public class ServerServiceImpl extends RemoteServiceServlet implements ServerService
{
	...
	public ServerServiceImpl ()
	{
		...
		if (currentDirectory.startsWith ("/"))
			...
			applicationPath = "C:\\workspace\\IRZ\\irzimbabwe\\war\\";
	...
	public SmsText findSmsTextByLanguage (String ruleId, int languageId)
	{
		return (SmsText) HibernateUtil.util.findObject ("from SmsText where ruleId='" + ruleId + "' AND languageId='" + languageId + "'");
	}
	...
	public String saveVisitPurpose (Visit visit, Patient patient, Encounter encounter, EncounterResults[] encounterResults)
	{
		...
		try
		{
			if (savePatient (patient) && saveVisit (visit))
			{
	...
	public String saveRegistration (Person person, Patient patient, Encounter encounter, EncounterResults[] encounterResults)
	{
		...
		try
		{
			...
			if (visit.getVisitPurpose ().equals ("TB"))
			{
	...
	public String saveSmearResult (SputumTest sputumTest, Encounter encounter, EncounterResults[] encounterResults)
	{
		String result = "";
		try
		{
