...
public class ReportUtil
{
	...
	try
	{
	    ...
	    String dest = IRZ.getResourcesPath() + String.valueOf(new Date().getTime()) + ".csv";
	...
	try
	{
	    ...
	    JasperReport jasperReport = JasperCompileManager.compileReport(IRZ.getReportPath() + fileName + (fileName.endsWith(".jrxml") ? "":".jrxml"));
	    JasperPrint print = JasperFillManager.fillReport(jasperReport, new HashMap(), resultSource);
	    String dest = IRZ.getResourcesPath() + String.valueOf(new Date().getTime()) + (export == true ? ".csv":".pdf");
	    // Delete file if existing
	    try
	    {
		File file = new File(dest);
	...
	{
	    Connection con = HibernateUtil.util.getSession().connection();
	    con.setCatalog(IRZ.getDatabaseName());
	    ...
	    String filePath = IRZ.getReportPath() + fileName + (fileName.endsWith(".jrxml") ? "":".jrxml");
    ...
    {
	String[][] reports;
	File dir = new File(IRZ.getReportPath());
