...
public class SmearResultsComposite extends Composite implements ClickHandler, ChangeHandler
{
	...
	resultsFlexTable.setHeight("100%");
	...
	clientIdFlexTable.setWidget(2, 1, smsAlertCheckBox);
	smsAlertCheckBox.setWidth("150%");
	...
	setRights(formName);
	createSputumResultsClientSms=false;
    }
	...
	Button[] buttons = { checkIdButton, closeButton, saveButton };
	for (int j = 0; j < buttons.length; j++)
	    buttons[j].addClickHandler(this);

	smsAlertCheckBox.addClickHandler(this);
    }
	...
	smsAlertCheckBox.setValue(false);
    }
	...
	else if ((dateTestedDateBox.getTextBox().getValue()).equals(""))
	    errorMessage.append("Date Tested: " + CustomMessage.getErrorMessage(ErrorType.EMPTY_DATA_ERROR) + "\n");
	else if ((smearResultComboBox).getSelectedIndex() == -1)
	    errorMessage.append("Smear Result: " + CustomMessage.getErrorMessage(ErrorType.EMPTY_DATA_ERROR) + "\n");
	else if (!(dateTestedDateBox.getTextBox().getValue()).equals("") && DateTimeUtil.isFutureDate(dateTestedDateBox.getValue()))
	    errorMessage.append("Date Tested is a future date: " + CustomMessage.getErrorMessage(ErrorType.INVALID_DATA_ERROR) + "\n");
	else if (!(dateTestedDateBox.getTextBox().getValue()).equals("") && DateTimeUtil.compareDateOnly(dateTestedDateBox.getValue(), currentTest.getDateCollected()) < 0)
	...
	if (validate())
	{
	    ...
	    service.saveSmearResult(currentTest, encounter, encounterResults.toArray(new EncounterResults[] {}), createSputumResultsClientSms, new AsyncCallback<String>() {
	...
	else if (sender == smsAlertCheckBox)
	{
	    clientId = IRZClient.get(clientIdTextBox);
	    load(true);
	    try
	    {
		service.getColumnData("person", "mobile", "pid='" + clientId.toUpperCase() + "'", new AsyncCallback<String[]>() {
		    public void onSuccess(String[] result)
		    {
			if (result.length > 0 && smsAlertCheckBox.getValue())
			{//mobile exists and the check box is true
			    createSputumResultsClientSms=true;
			    load(false);
