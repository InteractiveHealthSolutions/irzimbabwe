...
public class RegistrationComposite extends Composite implements ClickHandler, ChangeHandler, ValueChangeHandler<Boolean>
{
    ...
    private Label lblLanguage = new Label("Preferred Language:");
    private Label lblNICformat = new Label("Format: 12-123456-A-12 or 12-1234567-Z-12");
    private Label lblhivPatient = new Label("This client visited for HIV");
    ...
    private ListBox preferredLanguageComboBox = new ListBox();
    private final Label lblNewLabel = new Label("Select a Preferred language in which you want to receive SMS in other details");
	...
	lblhivPatient.setVisible(false);

	suspicionFlexTable.setWidget(2, 1, lblhivPatient);
	lblHivStatus.setWordWrap(false);
	suspicionFlexTable.setWidget(3, 0, lblHivStatus);
	hivStatusComboBox.setName("HIV_STATUS");
	suspicionFlexTable.setWidget(3, 1, hivStatusComboBox);
	...
	contactFlexTable.setWidget(3, 1, lblNewLabel);
	...
	otherFlexTable.setWidget(6, 0, lblLanguage);
	preferredLanguageComboBox.setName("PREFERRED_LANGUAGE");
	otherFlexTable.setWidget(6, 1, preferredLanguageComboBox);
	otherFlexTable.setWidget(7, 0, lblClientsNationalIdentity);
	nicTextBox.setMaxLength(15);
	otherFlexTable.setWidget(7, 1, nicTextBox);

	otherFlexTable.setWidget(8, 1, lblNICformat);
    ...
    {
	ListBox[] listBoxes = { patientTypeComboBox, addressProvidedComboBox, cityComboBox, hasCellPhoneComboBox, occupationComboBox, educationLevelComboBox, preferredLanguageComboBox };
	...
	primaryCellPhoneOwnerComboBox, primaryOwnerHouseholdComboBox, secondaryCellPhoneOwnerComboBox, secondaryOwnerHouseholdComboBox, tertiaryCellPhoneOwnerComboBox, tertiaryOwnerHouseholdComboBox,
	occupationComboBox, educationLevelComboBox, preferredLanguageComboBox };
	...
	tertiaryCellPhoneOwnerComboBox.setEnabled(false);
	tertiaryOwnerHouseholdComboBox.setEnabled(false);
	preferredLanguageComboBox.setEnabled(true);
	landlineNumberTextBox.setEnabled(false);
	otherOccupationTextBox.setEnabled(false);
	otherEducationLevelTextBox.setEnabled(false);
	...
	if (validate())
	{
		...
		if (reachedViaSmsCheckBox.getValue())
		{
	    ...
	    currentPerson.setPreferredLanguage(preferredLanguageComboBox.getValue(preferredLanguageComboBox.getSelectedIndex()));
	    encounterResults.add(new EncounterResults(new EncounterResultsId(eId, clientId, pid2, formName, "PREFERRED_LANGUAGE"), IRZ.getDefinitionKey("PREFERRED_LANGUAGE", preferredLanguageComboBox
	    .getValue(preferredLanguageComboBox.getSelectedIndex()))));
	    String nic = IRZClient.get(nicTextBox).toUpperCase();
	    if (!nic.equals(""))
	    {
		encounterResults.add(new EncounterResults(new EncounterResultsId(eId, clientId, pid2, formName, "NIC"), nic));
		currentPerson.setNationalId(nic);
	    }
		...
		}// end of onSucess result

		public void onFailure(Throwable caught2)
		{
		    caught2.printStackTrace();
		}
	    });
	}
    }
				...
				public void onSuccess(Patient result)
				{
					...
					try
					{
						...
						public void onSuccess(Visit result)
						{
						    ...
						    else if (result.getVisitPurpose().equals("TB") && !(currentPatient.getDiseaseSuspected().equals("TB")))
						    {
	...
	else if (sender == saveButton)
	{
	    ...
	    {
		try
		{ // See if the user is trying to update the patient info
		  // and if
		  // there is an existing encounter confirm
		    service.exists("encounter", "pid1='" + IRZClient.get(clientIdTextBox) + "' and encounter_type='TB_REG'", new AsyncCallback<Boolean>() {
			public void onSuccess(Boolean result)
			{
			    if (result)
			    {
	...
	else if (sender == clientIdTextBox)
	{
	    lblhivPatient.setVisible(false);
